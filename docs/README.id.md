...
[English](https://github.com/invoqmoney/sdk-ruby/blob/main/README.md) · **Bahasa Indonesia** · [Español](./README.es-419.md) · [Français](./README.fr.md) · [Português](./README.pt-BR.md) · [Tiếng Việt](./README.vi.md) · [Türkçe](./README.tr.md) · [ไทย](./README.th.md) · [简体中文](./README.zh-Hans.md) · [繁體中文](./README.zh-Hant.md)

> Dokumen ini terjemahan dari README bahasa Inggris; kalau ada perbedaan, [versi bahasa Inggris](https://github.com/invoqmoney/sdk-ruby/blob/main/README.md) yang berlaku.
...
SDK memeriksa bahwa nilai `amount` dan argumen `invoice_id` berupa string yang tidak kosong sebelum mengirim request. API invoq memvalidasi format dan rentang dari jumlah tersebut.

Jangan masukkan field opsional yang tidak diisi ke dalam hash request. Saat Anda menyertakan `description` atau `reference_id`, berikan sebuah string. `return_url` bisa berupa string atau `nil`. Kunci lain apa pun di dalam hash akan dibuang, bukan dikirim, karena API menolak key body yang tidak dikenal.
...
Kenali sebuah opsi pembayaran lewat `chain_namespace`, `chain_reference`, dan `token_address`, jangan lewat posisinya di dalam array. `monitoring_ends_at` adalah akhir jendela pembayaran, dan bernilai `nil` untuk invoice uji coba.
...
def handle_invoq_webhook(env)
  ...
  if Invoq.invoice_paid?(event)
    ...
    # Tahan atau batalkan pesanan untuk fulfillment_key.
  end
...
- Balas 2xx secepatnya. Status lain dihitung sebagai pengiriman gagal dan akan diulang — termasuk redirect dan `4xx` — jadi jendela deploy atau rute yang sempat salah akan diulang, bukan dibuang. Jedanya 1 menit, 5 menit, 30 menit, lalu 2 jam, sampai 5 kali percobaan.
- Urutan kedatangan pengiriman tidak dijamin. Simpan snapshot dengan `payment_revision` tertinggi.

`Invoq.invoice_paid?` menerima event `invoice.paid` yang bisa diproses dengan status invoice `paid`, `settling`, atau `settled`; fungsi ini menolak `review_required`. `Invoq.invoice_payment_reversed?` menerima event `invoice.payment_reversed` tanpa memeriksa statusnya sama sekali: pembatalan yang Anda buang akan menyisakan pesanan yang sudah diproses padahal pembayarannya sudah tidak ada. Tipe event yang belum dikenal versi SDK ini tetap lolos verifikasi dan dikembalikan apa adanya.
