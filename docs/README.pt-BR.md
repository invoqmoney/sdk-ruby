...
- `amount` é uma string decimal em USD de `"0.01"` a `"999.99"`, com até 2
...
`payment_status`, `project`, `deposit_address`, `monitoring_ends_at` e
`direct_onchain_rails`, mas não inclui `reference_id`. Use a resposta de criação
ou o webhook `invoice.paid` quando precisar do seu `reference_id` de comerciante.
...
usa a mesma escala de 18 casas decimais de `amount_paid`.
